    ...
    <aside className="hidden w-64 shrink-0 border-r border-gray-200 bg-white md:flex md:flex-col">
      <div className="border-b border-gray-200 px-7 py-7">
        <div className="flex items-center gap-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-md border border-teal-200 bg-teal-50 text-3xl font-bold text-[#00A99D]">
            V
          </div>
          <h1 className="text-xl font-extrabold leading-tight text-slate-800">
            AI-Powered
            <br />
            Pharmacy
            <br />
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.key}
              href={item.href}
              className={`flex items-center gap-4 px-8 py-3 text-sm font-bold transition ${
                isActive ? "bg-[#10b7ab] text-white shadow-sm" : "text-slate-700 hover:bg-gray-100"
              }`}
            >
              <Icon size={20} />
